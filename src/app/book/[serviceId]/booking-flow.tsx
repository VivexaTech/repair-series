...
export function BookingFlow({ serviceIdOrSlug }: { serviceIdOrSlug: string }) {
  ...
  const serviceCharge = displayPrice ?? 0;
  const visitingCharge = getVisitingCharge(service ?? ({} as ServiceDoc));
  const estimatedTotal = useMemo(() => {
    return serviceCharge + visitingCharge;
  }, [serviceCharge, visitingCharge]);
    ...
    <div className="mx-auto max-w-3xl pb-16">
      ...
      <div className="rounded-[24px] border border-white/40 bg-white/95 p-6 shadow-[0_20px_40px_rgba(0,0,0,0.06)] backdrop-blur-[12px] sm:p-8">
        ...
        {step === 2 ? (
          <div className="space-y-6">
            <div>
              <h2 className="text-xl font-bold text-[#0a0f1c]">Review booking</h2>
              <p className="mt-1 text-sm text-[#64748b]">
